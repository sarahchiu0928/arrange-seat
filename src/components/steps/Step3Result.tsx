...
function GuestRow({ g }: { g: GuestEntry }) {
  ...
  const countLabel = g.split
    ? `${g.effective}人（原${g.total}人）`
...
function DroppableTableCard({ table }: { table: TableType }) {
  const { setNodeRef, isOver } = useDroppable({ id: `t:${table.num}` })
    ...
    >
      ...
      <div className="space-y-1 mt-5">
        {table.guests.map((g, i) => (
          <DraggableGuestRow key={`g:${table.num}:${i}`} id={`g:${table.num}:${i}`} guest={g} />
        ))}
...
export function Step3Result() {
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    const t = tables.find((t) => t.num === parseInt(tableNum))
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    moveGuest(parseInt(fromNum), parseInt(idx), parseInt(toNum))
  }

  const { unassigned } = arrange(guests, settings)
